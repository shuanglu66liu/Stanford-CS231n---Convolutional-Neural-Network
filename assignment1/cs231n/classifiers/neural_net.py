...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    ind=np.ones(first_layer.shape)        
    ind[first_layer==0]=0
    for i in range(N):
        dW2[:,y[i]]+=-first_layer[i]
        dW2+=np.outer(first_layer[i,:],np.exp(scores[i,:])/np.sum(np.exp(scores[i,:])))
        db2+=np.exp(scores[i,:])/np.sum(np.exp(scores[i,:]))
        db2[y[i]]-=1
        dS=np.exp(scores[i,:])/np.sum(np.exp(scores[i,:]))
        dS[y[i]]=dS[y[i]]-1
        for j in range(W1.shape[1]):
            dW1[:,j]+=ind[i,j]*dS.dot(W2[j,:])*X[i,:]
            db1[j]+=ind[i,j]*dS.dot(W2[j,:])
    dW1=dW1/N+reg*W1
    dW2=dW2/N+reg*W2    
    grads['W1']=dW1
    grads['W2']=dW2
  ...
  def predict(self, X):
    ...
    first_layer=X.dot(W1)+b1
    first_layer[first_layer<=0]=0
    y_pred=np.argmax(first_layer.dot(W2)+b2,axis=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
